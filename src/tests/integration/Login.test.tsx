...
describe("Login Page", () => {
	it("should fill username, password, click login button, and response 200", async () => {
		// mock fetch agar tidak benar-benar ke network
		// ini solusi kalo gak pake MSW
	...
	it("should show error message when response 401", async () => {
		// mock fetch agar tidak benar-benar ke network
		// ini solusi kalo gak pake MSW
		global.fetch = jest.fn(() =>
			Promise.resolve({
		...
		fireEvent.click(loginButton);
		const alert = await findByText(/Invalid credentials/i);
